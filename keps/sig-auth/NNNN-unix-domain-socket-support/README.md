...
- Implement support for specifying unix socket addresses in rest.Config.
  - References to UDS backed servers are distinguished by a separate URI
    scheme, see below.
  - As a hardening measure, we propose the following Kubernetes specific
  ...
  - Since socket file names are local to the client environment, the exact file
    ...
    validation without requiring server certificates to be signed for the exact
    socket path, `rest.TLSClientConfig.ServerName` is required to be set when
    UDS server is specified.
...
socket address. Since socket addresses can contain path separators, literal `/`
